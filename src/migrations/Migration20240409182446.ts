...
export class Migration20240409182446 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table `product` (`id` int unsigned not null auto_increment primary key, `created_at` datetime not null, `updated_at` datetime not null, `category_id` int unsigned not null, `name` varchar(255) not null, `description` varchar(8192) not null default \'\', `squirrels` float not null default 0, `fats` float not null default 0, `carbohydrates` float not null default 0, `flags` smallint not null default 0, `calories` int not null default 0, `price` float not null, constraint product_price_check check (price >= 0)) default character set utf8mb4 engine = InnoDB;');
    ...
    this.addSql('create table `order` (`id` int unsigned not null auto_increment primary key, `created_at` datetime not null, `updated_at` datetime not null, `user_id` int unsigned not null, `address_id` int unsigned null, `status` varchar(255) not null default \'new\', `comment` text null, `payment_type` varchar(255) not null, `confirmation_token` varchar(255) not null default \'\') default character set utf8mb4 engine = InnoDB;');
    ...
    this.addSql('create table `order_item` (`order_id` int unsigned not null, `product_id` int unsigned not null, `amount` int not null default 1, primary key (`order_id`, `product_id`), constraint order_item_amount_check check (amount > 0)) default character set utf8mb4 engine = InnoDB;');
